...
impl SecureStorage {
    ...
    fn load_index(&self) -> Result<AccountsIndex> {
        ...
        if !path.exists() {
            return Ok(AccountsIndex { accounts: Vec::new() });
        }
        
        let content = fs::read_to_string(&path)
            .context("Failed to read accounts index")?;
        
        let index: AccountsIndex = serde_json::from_str(&content)
            .context("Failed to parse accounts index")?;
    ...
    fn save_index(&self, index: &AccountsIndex) -> Result<()> {
        let content = serde_json::to_string_pretty(index)
            .context("Failed to serialize accounts index")?;
        
        fs::write(self.index_path(), content)
            .context("Failed to write accounts index")?;
    ...
    pub fn store_credentials(&self, account_name: &str, credentials: &str) -> Result<()> {
        let entry = Entry::new("tokstat", account_name)
